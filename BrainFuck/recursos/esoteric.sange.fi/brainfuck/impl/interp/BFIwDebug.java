...
/*
 * Program Name:  BFIwDebug
 * Version:       1.0
 * Date:          2003-03-18
 * Description:   Interpreter for the Brainfuck Programming Language
 *                Includes to '#' debugging command.
 * License:       GPL
 * Web page:      http://www.brainfuck.ca
 * Download:      http://www.brainfuck.ca/BFIwDebug.java
...
/**
 *  BFIwDebug includes the '#' operator for debugging purposes
 *  @author Thomas Cort (<A HREF="mailto:tom@tomcort.com">tom@tomcort.com</A>)
 *  @version 1.1 2003-03-16
 *  @since   1.0
 */
public class BFIwDebug {

  protected final int MAX_DATA_SIZE = 32767;
  protected byte [] x;
  protected char [] c;
  ...
  /**
   *  Constructor
   *  @param String - Brainfuck program to interpret.
   *  @since 1.0
   */
  public BFIwDebug (String s) {
  ...
  public void interpret() {
    for (pc = 0; pc < c.length ; pc++) {
      if      (c[pc] == '>') incrementPointer();
      else if (c[pc] == '<') decrementPointer();
      else if (c[pc] == '+') incrementByteAtPointer();
      else if (c[pc] == '-') decrementByteAtPointer();
      else if (c[pc] == '.') output();
      else if (c[pc] == ',') input();
      else if (c[pc] == '[') startJump();
      else if (c[pc] == ']') endJump();
      else if (c[pc] == '#') debugDump();
    }
  }
  
  /**
   *  '#' dumps the values at locations 0 thru 9
   *  @since 1.0
   */
  ...
  public void checkSyntax() {
    int lt = 0, gt = 0, ob = 0, cb = 0, ptrAt = 0;
    ...
    if (ob != cb)
      errBF( ob>cb ? "Missing closing bracket(s)":"Missing opening bracket(s)");
  }
